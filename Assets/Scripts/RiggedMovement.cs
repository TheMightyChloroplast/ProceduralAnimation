...
public class RiggedMovement : MonoBehaviour
{
    ...
    {     //initialises the current positions of the feet prior to start of animation.
        ...
        //left foot is half to offset feet and ensure different movementS
    ...
    void Update()
    {
        ...
        if (_timeStep < 1)
        {
            ...
            _currentLeftFootPos =
                Vector3.Lerp(_lastLeftFootPos, _nextLeftFootPos,
                    _timeStep); //lerp between the previous and the next position to get the current position
            _currentLeftFootPos.y +=
                Mathf.Sin(Mathf.PI * _timeStep) *
                stepHeight; //does a Sin function to get a curve of the step and applies it to the y axis.
        ...
        else if (_timeStep < 2)
        {
            ...
            _currentRightFootPos =
                Vector3.Lerp(_lastRightFootPos, _nextRightFootPos,
                    (_timeStep - 1)); //lerp between the previous and the next position to get the current position
            _currentRightFootPos.y +=
                Mathf.Sin(Mathf.PI * (_timeStep - 1)) *
                stepHeight; //does a Sin function to get a curve of the step and applies it to the y axis.
